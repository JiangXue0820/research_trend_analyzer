...
class PaperSummarizer:
    def __init__(
        ...
        if not self.scope_list_path.is_file():
            msg = f"Scope list file does not exist: {self.scope_list_path}"
            logging.warning(msg)
            raise ValueError(msg)
        
        if not self.paper_list_root.is_dir():
            msg = f"Paper list directory does not exist: {self.paper_list_root}"
            logging.error(msg)
            raise ValueError(msg)
    ...
    def load_paper_list(self, raw_list_path: str) -> List[Dict[str, Any]]:
        ...
        if not path.is_file():
            msg = f"Paper list file does not exist: {path}"
            logging.error(msg)
            ...
            for i, line in enumerate(f, 1):
                ...
                try:
                    ...
                    if isinstance(obj, dict):
                        ...
                        logging.warning(f"Non-dict row at line {i} in {path}; skipped.")
                except json.JSONDecodeError:
                    logging.warning(f"Malformed JSON at line {i} in {path}; skipped.")
        return papers
    ...
    def get_paper_content(self, paper_title: str, paper_url: str) -> Dict[str, Any]:
        ...
        if not paper_url:
            return make_response("warning", f"No paper URL for '{paper_title}'.", None)
        if not paper_title:
            return make_response("warning", "Paper title is empty.", None)
        ...
        try:
            ...
            if res.get("status") == "success":
                msg = f"Extracted content for '{paper_title}', {res.get('message', '')} ({len(res.get('data', '') or '')} chars)."
                logging.info(msg)
            ...
            msg = f"Exception during PDF processing: {e}"
    ...
    def make_summary(self, paper_title: str, authors: str, paper_content: str, keywords: str, prompt: str) -> Dict[str, Any]:
        ...
        if not paper_content:
            return make_response("warning", f"No paper content for '{paper_title}'.", None)
        if not prompt:
            return make_response("warning", f"No prompt provided for '{paper_title}'.", None)
        ...
        try:
            ...
            if not isinstance(rsp, dict) or rsp.get("status") != "success":
                if "RESOURCE_EXHAUSTED" in rsp.get("message", ""):
                    msg = "LLM resource exhausted; stopping further calls."
                    logging.error(msg)
                    raise RuntimeError(msg)
                msg = f"LLM call failed: {rsp.get('message', 'unknown error') if isinstance(rsp, dict) else rsp}"
                logging.error(msg)
                return make_response("error", msg, None)

            response_text = rsp.get("data", "")
            if not isinstance(response_text, str) or not response_text.strip():
                msg = f"Empty or non-string LLM response: {response_text!r}"
                logging.error(msg)
                return make_response("error", msg, None)

            return make_response("success", "Paper summarized successfully.", response_text)
        except Exception as e:
            msg = f"Exception during LLM call: {e}"
            logging.exception(msg)
            return make_response("error", msg, None)
    ...
    def summarize_papers(
        ...
        """
        if topic:
            logging.info(f"Summarizing papers for {conference}, {year}, {topic}")
        else:
            logging.info(f"Summarizing papers for {conference}, {year}")
        ...
        try:
            ...
            msg = f"Error loading scope list file '{self.scope_list_path}': {e}"
            logging.error(msg)
        ...
        if not raw_list_path.exists():
            raise FileNotFoundError(f"Paper list not found: {raw_list_path}")

        papers = self.load_paper_list(str(raw_list_path))
        if not papers:
            raise ValueError(f"No papers in: {raw_list_path}")
        ...
        for paper in tqdm(papers, desc="Summarizing papers"):
            ...
            if content_rsp.get("status") != "success":
                logging.warning(f"Failed to get paper content for '{title}': {content_rsp.get('message')}")
                fail += 1
                continue
            ...
            for suf in missing:
                ...
                if summary_rsp.get("status") != "success":
                    logging.warning(f"Failed to make summary for '{title}' ({suf}): {summary_rsp.get('message')}")
                    fail += 1
                    continue

                summary = summary_rsp.get("data", "")
                if not summary:
                    logging.warning(f"Empty summary for '{title}' ({suf})")
                    fail += 1
                    continue
                ...
                added += 1
                logging.info(f"Saved {suf} summary for '{title}' → {targets[suf]}")
                
        logging.info(f"Finished summarizing: {added} added, {fail} failed")
